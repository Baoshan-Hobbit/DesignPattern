/*************************************************************************
  > TransparentFile Name: structure.h
  > Author: baoshan
  > Mail: baoshanw@foxmail.com 
  > Created Time: 2020年03月12日 星期四 16时47分07秒
 ************************************************************************/

#ifndef CPLUS_PRACTICE_DESIGN_PATTERN_STRUCTURE_H_
#define CPLUS_PRACTICE_DESIGN_PATTERN_STRUCTURE_H_
...
class Proxy : public Subject {
 ...
 private:
  void PreRequest() { printf("Before request.\n"); }
  void AfterRequest() { printf("After request.\n"); } 
...
class Decorator : public Component {
 ...
 protected:
  ...
  void OperateComponent() { component_->Operate(); }
...
class FlyWeight {
 ...
 protected:
  explicit FlyWeight(const std::string& internal) { internal_ = internal; }
...
class ConcreteFlyWeight : public FlyWeight {
 public:
  explicit ConcreteFlyWeight(const std::string& internal) 
    : FlyWeight(internal) {}
...
class CachedFactory {
 public:
  ...
  size_t get_count() const { return fly_weight_pool_.size(); }

 private:
  CachedFactory() {} // 一定要定义,否则会报错"未定义的引用"
  std::map<std::string, FlyWeight*> fly_weight_pool_;
};

#endif // CPLUS_PRACTICE_DESIGN_PATTERN_STRUCTURE_H_
