/*************************************************************************
  > File Name: behavior.cc
  > Author: baoshan
  > Mail: baoshanw@foxmail.com 
  > Created Time: 2020年03月19日 星期四 19时20分30秒
 ************************************************************************/

#include "src/design_pattern/behavior.h"
#include <stdio.h>
#include <map>
#include <string>
...
void ConcretePublisher::set_state(int state) { state_ = state; }
int ConcretePublisher::get_state() const { return state_; }
ConcreteSubscriber::ConcreteSubscriber(const std::string& name, 
		...
		: name_(name), 
			Subscriber(publisher_register), 
			publisher_content_(publisher_content) {}
void ConcreteSubscriber::Update() {
	int state = publisher_content_->get_state();
	printf("subscriber name: %s, publisher state: %d\n", name_.c_str(), state);
}

void Receiver::On() { printf("Receiver on.\n"); }
void Receiver::Off() { printf("Receiver off.\n"); }
void Receiver::Alter() { printf("Receiver alter.\n"); }
void OnCommand::Execute() { receiver_->On(); }
void OffCommand::Execute() { receiver_->Off(); }
void AlterCommand::Execute() { receiver_->Alter(); }
void Invoker::Add(Command* command) { command_list_.push_back(command); }
...
void Context::Close() { state_->OnClose(this); }
...
void Leader::HandleRequest(Request* request) {
	if (request->get_type() == "vocation" && request->get_content() < 3) {
		printf("%s approved: %s request %d vocation\n", name_.c_str(), 
					request->get_name().c_str(), request->get_content());
	} else {
...
void ConcreteElementA::Accept(Visitor* visitor) { visitor->Visit(this); }
void ConcreteElementB::Accept(Visitor* visitor) { visitor->Visit(this); }
void ObjectStructure::Accept(Visitor* visitor) {
	for (std::vector<Element*>::const_iterator it=element_list_.begin(); 
			it!=element_list_.end(); 
			++it) 
		(*it)->Accept(visitor);
}
